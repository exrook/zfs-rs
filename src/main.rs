...
fn main() {
    println!("Hello, world!");
    let dev = Disk::new(args_os().nth(1).expect("An argument is required"));
    rocket::ignite()
        .manage(dev)
...
fn block(dev: State<Disk>, vdev: u32, id: NumFromHex, asize: u32, view: Option<String>) -> String {
...
fn block_compressed(
    ...
    let address = id.0 + 0x400000;
    let compressed_data = dev.read(address, phys_size as u64 * 512).unwrap();
